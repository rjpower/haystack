...
ALL_LAYERNORM_LAYERS.append(HaystackRMSNorm)


class HaystackRotaryEmbedding(nn.Module):
...
# Copied from transformers.models.mistral.modeling_mistral.MistralMLP with Mistral->Haystack
class HaystackMLP(nn.Module):
...
class NNDBLookup(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor):
        ...
        # Original hidden_states shape: (batch_size, seq_len, hidden_size)
        # New hidden_states shape: (batch_size, seq_len + key_context, hidden_size)

        # Compute lookup keys for the database by sliding a window over `hidden_states`.
...
class HaystackAttention(nn.Module):
    ...
    # Ignore copy
    def __init__(self, config: HaystackConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
        ...
        self.rotary_emb = HaystackRotaryEmbedding(
            self.head_dim,
...
class HaystackSdpaAttention(HaystackAttention):
    ...
    def forward(
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "HaystackModel is using HaystackSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True`. Falling back to the manual attention implementation, "
...
class HaystackDecoderLayer(nn.Module):
    def __init__(self, config: HaystackConfig, layer_idx: int):
        ...
        self.post_attention_layernorm = HaystackRMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
        self.db_layer = NNDBLookup(config)
    ...
    def forward(
        ...
        **kwargs,
        ...
        # augment context window with output from the db lookup
        residual = hidden_states
        db_lookup = self.db_layer(hidden_states)
        hidden_states = torch.cat([db_lookup, hidden_states], dim=1)

        # pad out residual with zeros to fit the shape of hidden_states
        residual = F.pad(
            residual, (0, 0, 0, hidden_states.shape[1] - residual.shape[1])
        )
        ...
        # Remove the db lookup from the hidden states
        hidden_states = hidden_states[:, db_lookup.shape[1] :, :]
...
class HaystackPreTrainedModel(PreTrainedModel):
    ...
    _no_split_modules = ["HaystackDecoderLayer"]
...
class HaystackModel(HaystackPreTrainedModel):
    ...
    def __init__(self, config: HaystackConfig):
        ...
        self.db = NNDBLookup(config)
        self.layers = nn.ModuleList(
            [
                HaystackDecoderLayer(config, layer_idx)
                for layer_idx in range(config.num_hidden_layers)
            ]
        )
        self.norm = HaystackRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
    ...
    def _update_causal_mask(
        self, attention_mask, batch_size, seq_length, dtype, device, input_tensor
    ):
...
# Copied from transformers.models.llama.modeling_llama.LlamaForCausalLM with LLAMA->HAYSTACK,Llama->Haystack,llama->gemma
class HaystackForCausalLM(HaystackPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = HaystackModel(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        ```python
        >>> from transformers import AutoTokenizer, HaystackForCausalLM

        >>> model = HaystackForCausalLM.from_pretrained("google/gemma-7b")
